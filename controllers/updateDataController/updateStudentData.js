...
export const updateStudentData = asyncHandler(async (req, res) => {
  try {
    upload.fields([{ name: 'student_profile_update_photo', maxCount: 1 }])(
      req,
      res,
      async (err) => {
        if (err) {
        ...
        const student_photo_update_url = req.files[
          'student_profile_update_photo'
        ]
          ? req.files['student_profile_update_photo'][0].path
          : null;
        ...
        if (student_photo_url) {
          ...
          paramIndex++;
        } else if (req.files['student_profile_update_photo']) {
          query += `student_photo_url = $${paramIndex}, `;
          params.push(student_photo_update_url);
          paramIndex++;
        }
        ...
        res.status(200).json({
          ...
          student_photo_url: req.files['student_profile_update_photo']
            ? student_photo_update_url
            : student_photo_url,
        });
      },
    );
