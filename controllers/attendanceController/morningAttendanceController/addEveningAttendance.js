...
// @route POST /api/attendance/addEveningAttendance
// @access public

export const addEveningAttendance = asyncHandler(async (req, res) => {
  try {
    ...
    const query = `
          INSERT INTO "eveningAttendance" (pin_number, date, status)
          VALUES ($1, $2, $3)
    ...
    res.status(201).json({
      message: 'evening attendance added or updated successfully',
      attendance_number: response.rows[0]?.attendance_number, // Return the attendance_number
    });
  } catch (error) {
    console.error(error);
    res.status(500).send('Error adding or updating evening attendance');
  }
});
